...
export async function copyDefaultConfigIfMissing(params = {}) {
  ...
  const sourcePath = path.resolve(
    params.sourcePath ?? path.join(scriptDir, "..", "md-context-audit.config.example.json")
  );
  const destinationRoot = path.resolve(params.destinationRoot ?? process.env.INIT_CWD ?? process.cwd());
  const destinationPath =
    path.resolve(params.destinationPath ?? path.join(destinationRoot, "md-context-audit.config.json"));
...
async function main() {
  try {
    ...
    console.warn(`md-context-audit: unable to copy default config: ${message}`);
  }
}
