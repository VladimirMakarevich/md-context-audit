...
- `--config <file>`: path to `md-context-audit.config.(json|cjs|mjs)`;
...
- Default behavior for `scan`:
  ...
  - therefore `md-context-audit scan` exits non-zero with the default `--fail-on error`;
...
`md-context-audit.config.json|cjs|mjs`:
