# md-context-audit v2 — Production Roadmap
...
The target is a substantially larger product (**`md-context-audit`**):

- a **`@md-context-audit/core`** engine with a **registry of 21 schema-validated
...
- a **`@md-context-audit/cli`** with `lint` (default) · `init` · `graph` · `slice`
  · `impact` · `compile`;
- a **`@md-context-audit/mcp-server`** exposing 6 deterministic tools over stdio;
...
Single package `md-context-audit`, Node 24.17 LTS, ESM, TypeScript NodeNext.
...
```
@md-context-audit/core        ← parser, ParsedDocument, ContextGraph, rule engine,
                                 registry (21 rules), config, compiler, formatters
        ├── @md-context-audit/cli         ← commander: lint|init|graph|slice|impact|compile
        ├── @md-context-audit/mcp-server  ← stdio: 6 tools
        └── (optional) @md-context-audit/lsp-server   ← stretch / out of v2 scope
skills/                       ← md-context-audit-{init,fix,impact}/SKILL.md (agentskills.io)
...
workspaces). See Decision D1. Naming throughout: bins `md-context-audit` and
`md-context-audit-mcp`, config `md-context-audit.config.json`, org/repo
`VladimirMakarevich/md-context-audit` (replace any leftover `contextlint` or
...
- Config `compile` section (skill name/description, section flags); CLI `compile`
  with `--outdir` / `--dry-run`, default `.claude/skills/md-context-audit/`.
...
- Interactive (`@inquirer/prompts`): language, include patterns, rule categories →
  writes `md-context-audit.config.json` with a sensible zero-config rule set.
...
- `@md-context-audit/mcp-server`: stdio transport, 6 tools — `lint`, `lint-files`,
...
- README + host config snippet (`npx @md-context-audit/mcp-server`).
...
- `skills/md-context-audit-{init,fix,impact}/SKILL.md` with frontmatter
