...
3. Ship `schema.json` inside the published package; default `$schema` is a **relative local
   path** (`./node_modules/@md-context-audit/cli/schema.json`).
4. `md-context-audit schema` command writes a **project-local** schema (incl. custom-rule
...
- [ ] `md-context-audit schema` writes a local project schema; no remote URL anywhere.
