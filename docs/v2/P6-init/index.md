...
> set, and writes a valid `md-context-audit.config.json` with a local `$schema` — so the CLI
...
- [ ] Writes a valid `md-context-audit.config.json` (canonical IDs, optional rationale
