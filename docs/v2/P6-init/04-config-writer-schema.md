...
1. Write `md-context-audit.config.json` in `cwd`: `$schema` (local path), `include`/`exclude`,
...
3. Optional ([I6](../requirements/06-installation.md)): offer to drop
   `.github/workflows/md-context-audit.yml` referencing the published GitHub Action (P9) —
