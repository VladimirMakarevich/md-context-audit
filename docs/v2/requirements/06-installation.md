...
| **I6** | First-class GitHub Action / reusable CI workflow | ✅ Accepted | Publishable Action; `init` can drop `.github/workflows/md-context-audit.yml`. |
...
- **I2 — smart CLI `init`.** The reference splits a *dumb* manual `init` (no scanning,
  ...
  for CI. The `md-context-audit-init` skill then orchestrates and layers GH Actions + README
  on top (S7/S8).
...
- **I4 — single-tag release.** Per [vendor-neutral skill distribution](../decisions/vendor-neutral-skill-distribution.md),
  one `vX.Y.Z` tag publishes `@md-context-audit/{core,cli,mcp-server}` and tags the skills
...
- **I7 — skill install.** Document
  `gh skill install VladimirMakarevich/md-context-audit <skill> --pin vX.Y.Z`; the skill
