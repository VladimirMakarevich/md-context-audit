...
| **R8** | Inline suppression directives | ✅ Accepted | `<!-- md-context-audit-disable[-next-line] RULE-ID -->`. Parser captures comments + positions. |
...
declarative-rule vocabulary (R9 Tier 1). `init`/`md-context-audit schema` writes a
...
- **CLI (P6+):** `--fix` flag (R2); `md-context-audit schema` command (C9).
