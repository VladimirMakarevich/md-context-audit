...
> static skills (`skills/md-context-audit-{init,fix,impact}/`). Locked v2 requirement;
...
| **S7** | Host-neutral static skills + replace upstream placeholders | B | ✅ Accepted | Swap `vladimir-makarevich` / `md-context-audit.dev` for our repo; no Claude-specific syntax (the vendor-neutral skill distribution decision). |
...
- **S2 — host-neutral commands.** The generated skill hardcodes
  `!npx md-context-audit impact $ARGUMENTS` (Claude-Code command-injection style). The
...
- **S7 — host-neutral + placeholders.** Replace upstream `vladimir-makarevich` /
  `md-context-audit.dev` with `VladimirMakarevich/md-context-audit`; keep all 3 skills
