...
```jsonc
{
  "$schema": "./node_modules/@md-context-audit/cli/schema.json",   // C9 — local, version-matched, offline; NO remote URL
  ...
  "compile": {
    "outdir": ".claude/skills/md-context-audit",
...
- **C9 — schema delivery (no remote URL).** Pinning `$schema` to a raw GitHub URL on
  ...
  2. defaults `$schema` to a **relative local path**
     (`./node_modules/@md-context-audit/cli/schema.json`) — offline + version-matched;
  3. provides a generator (`md-context-audit schema`, also run by `init` when custom
