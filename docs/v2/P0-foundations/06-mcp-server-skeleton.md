# P0.06 · `@md-context-audit/mcp-server` package skeleton (stub)
...
Create the `@md-context-audit/mcp-server` package shell so the 3-package workspace shape and
...
1. `packages/mcp-server/package.json`: name `@md-context-audit/mcp-server`, bin
   `md-context-audit-mcp` → `dist/index.js`, deps `@md-context-audit/core` (`workspace:*`),
