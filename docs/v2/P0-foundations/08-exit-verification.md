...
- **Next:** **Phase P1 — `ParsedDocument` & parser upgrade** (see the [roadmap](../Index.md));
  it extends the parser now living in `@md-context-audit/core`.
...
1. Run the full gate: `npm run typecheck && npm test && npm run build` across the workspace —
   all green.
2. **Parity check:** run `md-context-audit scan` and `graph` on a fixture (and on this repo)
...
token estimator all live in `@md-context-audit/core`, ready to be extended into the richer
