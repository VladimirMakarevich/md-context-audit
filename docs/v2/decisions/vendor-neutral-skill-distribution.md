...
Ship Agent Skills under `skills/<skill-name>/SKILL.md` in the `md-context-audit` repo,
following the agentskills.io specification.

- The repo itself is the distribution channel — no separate marketplace repo.
- Users install with `gh skill install VladimirMakarevich/md-context-audit <skill> [--pin vX.Y.Z]`.
