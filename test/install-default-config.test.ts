...
async function createTempDir(): Promise<string> {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "md-context-audit-install-"));
...
describe("copyDefaultConfigIfMissing", () => {
  it("copies the default config when the destination does not exist", async () => {
    ...
    const sourcePath = path.join(sourceDir, "md-context-audit.config.json");
    const destinationPath = path.join(destinationDir, "md-context-audit.config.json");
