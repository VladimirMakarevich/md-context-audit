...
async function createTempRepo(): Promise<string> {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "md-context-audit-config-"));
...
describe("loadConfig", () => {
  ...
  it("prefers an explicit config path over discovered files", async () => {
    const repoRoot = await createTempRepo();
    const discoveredConfigPath = path.join(repoRoot, "md-context-audit.config.json");
  ...
  it("loads json config files", async () => {
    const repoRoot = await createTempRepo();
    const configPath = path.join(repoRoot, "md-context-audit.config.json");
  ...
  it("loads cjs config files", async () => {
    const repoRoot = await createTempRepo();
    const configPath = path.join(repoRoot, "md-context-audit.config.cjs");
  ...
  it("loads mjs config files", async () => {
    const repoRoot = await createTempRepo();
    const configPath = path.join(repoRoot, "md-context-audit.config.mjs");
  ...
  it("rejects explicit ts config files in v1", async () => {
    const repoRoot = await createTempRepo();
    const configPath = path.join(repoRoot, "md-context-audit.config.ts");
  ...
  it("supports explicit Windows-like absolute config paths", async () => {
    ...
    const configPath = path.join(windowsRoot, "md-context-audit.config.mjs");
